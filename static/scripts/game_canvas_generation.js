...
Game.funcs.generate_element_canvas = function(canvas, element) {
	...
	let fg_colors = null;
	try {
		 bg_colors = element.fields.bg_colors.split(",")
		 fg_colors = element.fields.fg_colors.split(",")
...
Game.funcs.add_element = function(element) {
	...
	if ($("canvas[data-pk=" + element.pk + "]:not(.canvas-draw-queue)").length != 0) {
		return
	}
	...
	if (element.incomplete) {
		canvas = $("<canvas/>")
			...
			.attr("data-password", element.fields.password)
			.addClass("canvas-draw-queue")
			.attr("width", 50)
			.attr("height", 50)
	...
	// If there's no canvas, then create it
	canvas = $("canvas[data-pk=" + element.pk + "]")
	if (!canvas.length)
	{		
		canvas = $("<canvas/>")
	}
    ...
    canvas.each((idx, object) => {
        Game.funcs.generate_element_canvas(object, element)
    })
}